const fs = require('fs')
const { Functions: Func, Api } = new (require('@im-dims/wb'))

global.Apic = Api
global.header = `mari-wabot v${require('../../package.json').version}`
global.footer = Func.Styles('mari-wabot by Im-Dims')
...
global.status = Object.freeze({
  ...
  premium: Func.texted('bold', 'Only premium users are allowed, Baka (*￣ii￣)'),
  owner: Func.texted('bold', 'This command only for owner.'),
  mod: Func.texted('bold', 'This command only for moderator.'),
  ...
  register: Func.texted('bold', 'Hello sensei, Please register first to use this feature.'),
...
global.APIKeys = {
  'https://skizo.tech/': 'yourkey'
}

global.key = {
  groq: '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'
}
