...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      conn.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, want to upgrade to premium plan ? send *${usedPrefix}premium* to see benefit and prices.`, m, { largeThumb: true, thumbnail: 'https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg', url: setting.link }).then(() => chats.lastchat = new Date() * 1)
        continue
      }
        ...
        const btns = [
          {
            ...
            name: 'quick_reply',
            buttonParamsJson: JSON.stringify({
              display_text: 'Register Via Emails',
              id: `${usedPrefix}regmail`
            })
          }
        ]
        conn.sendIAMessage(m.chat, btns, m, {
          content: global.status.register,
          footer: global.footer
        })
      ...
      if (plugin.game && setting.game == false) { // game mode
        m.reply(global.status.game)
        continue
      }
      
      if (plugin.rpg && setting.rpg == false) { // RPG mode
    ...
    switch (action) {
      ...
      if (group.welcome) {
        ...
        for (let user of participants) {
          ...
          try {
           bpp = await conn.profilePictureUrl(user, 'image')
          } catch (e) {
          } finally {
