const { smsg, Functions: Func, Scraper, Print } = new (require('@im-dims/wb'))
    ...
    try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
        return conn.reply(m.chat, Func.texted('italic', 'Bot time has expired and will leave from this group, thank you :).', null, {
          mentions: participants.map(v => v.id) })).then(async () => {
            groupSet.expired = 0
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!m.isGroup && !['miscs-owner.js', 'miscs-price.js', 'user-reg.js'].includes(name) && chats && !isPrems && !users.banned && setting.groupmode) {
            conn.sendMessage(m.chat, `⚠️ Using bot in private chat only for premium user, want to upgrade to premium plan ? send *${usedPrefix}premium* to see benefit and prices.`, m).then(() => chats.lastchat = new Date() * 1)
            continue
          }
          ...
          if (plugin.premium && !isPrems) {
            conn.reply(m.chat, global.status.premium)
            continue
          }
          ...
          if (plugin.level > users.level) {
            conn.reply(m.chat, `level ${plugin.level} is required to use this command. Your level ${users.level}`, m)
            continue
          }
        ...
        if (group.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              text = (action === 'add' ? (group.sWelcome || conn.welcome || 'Welcome, @user!').replace('@subject', await conn.getName(id)).replace('@desc', groupMetadata.desc.toString()) :
              (group.sBye || conn.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0])
              conn.sendMessageModify(id, text, null, { largeThumb: true, thumbnail: pp, url: global.db.data.setting.link })
            }
          }
        }
        break
      case 'promote':
        text = (group.sPromote || conn.spromote || '@user ```is now Admin```')
      case 'demote':
        if (!text) text = (group.sDemote || conn.sdemote || '@user ```is no longer Admin```')
