...
module.exports = {
  run: async (m, { conn, usedPrefix: _p, command, text, args, env, setting, Func }) => {
    try {
      ...
      const message = setting.msg.replace('+tag', `@${m.sender.replace(/@.+/g, '')}`).replace('+name', m.name).replace('+greeting', Func.greeting()).replace('+db', (/mongo/.test(env.databaseurl) ? 'Mongo' : `Local : ${local_size}`))
      if (style === 1) {
        let tags = {}
        const defaultMenu = {
          before: `${message}\n`.trimStart(),
        ...
        let after = conn.menu.after || (conn.user.jid == global.conn.user.jid ? '' : `Powered by https://wa.me/255734980103`) + defaultMenu.after
        let _text = [
          before,
        ...
        conn.sendMessageModify(m.chat, Func.Styles(text.trim()), m, {
          ...
          conn.sendFMusic(m.chat, music, "Information About Bots | Ssa Team", "120363193509141242@newsletter", m)
        })
      } else if (style === 2) {
        const defaultMenu = {
          before: ``.trimStart(),
        ...
        let arrayMenu = ['admin', 'converter', 'downloader', 'fun', 'game', 'group', 'miscs', 'user', 'tools', 'voice', 'database', 'owner']
        ...
        if (teks == '404') {
          return conn.sendIAMessage(m.chat, buttons, m, {
            ...
            let music = await Func.random(['https://imgur.com/q7WXO5w.jpeg', 'https://imgur.com/q7WXO5w.jpeg', 'https://imgur.com/q7WXO5w.jpeg'])
            conn.sendFMusic(m.chat, music, "Information About Bots | LAZACK Team", "120363193509141242@newsletter", m)
          })
        }
        let groups = {}
        for (let tag in tags) {
