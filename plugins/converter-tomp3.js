      ...
      try {
         if (m.quoted && typeof m.quoted.buttons != 'undefined' && typeof m.quoted.videoMessage != 'undefined') {
            ...
            if (/ogg/.test(mime)) {
               ...
               m.react('🕒')
               const buff = await toAudio(await q.download(), 'mp3')
               if (/tomp3|toaudio/.test(command)) return conn.sendFile(m.chat, buff.data, 'audio.mp3', '', m)
               if (/tovn/.test(command)) return conn.sendFile(m.chat, buff.data, 'audio.mp3', '', m, {
                  ptt: true
               })
            } else {
