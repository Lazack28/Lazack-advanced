let handler = async (m, { conn, bot, text, usedPrefix, command, Func }) => {
  ...
  if (!text && !m.quoted)
    return conn.reply(m.chat, Func.example(usedPrefix, command, '@tag'), m)
  // let exists = await conn.isOnWhatsApp(number)
  // if (exists) return conn.reply(m.chat, `*Nomor target tidak terdaftar di WhatsApp*`, m)
  if (isNaN(number)) return conn.reply(m.chat, `*Nomor tidak valid!!.*`, m)
  if (number.length > 15)
    return conn.reply(m.chat, `*Format tidak valid!!.*`, m)
  try {
    if (text) {
    ...
    if (typeof global.db.data.users[user] == 'undefined') return m.reply('*Tidak terdaftar di database.*')
    if (global.db.data.users[m.sender].pasangan != '' && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user) {
      ...
      if (global.db.data.users[pacar].pasangan == user) {
        var denda = Math.ceil((global.db.data.users[m.sender].exp / 1000) * 20)
        global.db.data.users[m.sender].exp -= denda
        if (m.sender == pacar && global.db.data.users[m.sender].pasangan == user) return conn.reply(m.chat, `Kamu sudah berpacaran dengan @${global.db.data.users[m.sender].pasangan.split("@")[0]}\n\nsetia dong!\ndenda : ${format(denda)} (20%)`, m, {
          contextInfo: {
            mentionedJid: [user, global.db.data.users[m.sender].pasangan],
          },
        })
        conn.reply(m.chat, `Tau sopan santun dikit teman\n@${user.split("@")[0]} sudah berpacaran dengan @${pacar.split("@")[0]}\n\nSilahkan cari pasangan lain aja!\ndenda : ${format(denda)} (10%)*`, m, {
          contextInfo: {
